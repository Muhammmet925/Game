...
Button(parent=start_menu, text='DUNYA SEKTORU', scale=(0.2, 0.05), x=-0.15, y=0.05, color=color.dark_gray, on_click=lambda: set_map("dunya"))
Button(parent=start_menu, text='UZAY SEKTORU', scale=(0.2, 0.05), x=0.15, y=0.05, color=color.dark_gray, on_click=lambda: set_map("uzay"))
...
def setup_environment():
    if selected_map == "dunya":
        ...
        ground.color = color.green  # Hata burada düzeltildi
        ...
        for i in range(100):
            Entity(model='sphere', color=color.white, scale=random.uniform(0.05, 0.1),
                   position=Vec3(random.uniform(-100, 100), random.uniform(20, 50), random.uniform(-100, 100)), unlit=True)
...
class AlienPlant(Entity):
    def __init__(self, position):
        if selected_map == "dunya":
            ...
            # UZAYLI BİTKİ ÖRTÜSÜ VE DEV MANTARLAR
...
class AlienCreature(Entity):
    ...
    def update(self):
        ...
        if distance(self.position, player.position) < (5 if self.is_boss else 2):
            damage_player(0.2 if self.is_boss else 0.05)

# --- 5. OYUNCU VE UI ---
player = FirstPersonController(y=2, speed=12, enabled=False)
sword = Entity(parent=camera, model='cube', scale=(.02, .02, 1.3), position=(.5, -.4, .8), color=color.cyan, unlit=True)
...
def toggle_menu():
    ...
    player.enabled = not menu_parent.enabled
